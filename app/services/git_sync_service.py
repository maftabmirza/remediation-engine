...
class GitSyncService:
    ...
    def _process_file(self, file_path: Path, repo_url: str, app_id: Optional[UUID], user_id: Optional[UUID], branch: str = "main", force_update: bool = False):
        ...
        # Valid doc_types: architecture, api_spec, runbook, sop, troubleshooting, design_doc, postmortem, onboarding
        ext = file_path.suffix.lower()
        if ext in ['.md', '.txt', '.markdown']:
            doc_type = "design_doc"
            title = file_path.stem.replace("-", " ").title()
        elif ext in ['.yaml', '.yml', '.json', '.toml', '.xml', '.properties', '.dockerfile'] or file_path.name.lower() == 'dockerfile':
            doc_type = "design_doc"  # Config files stored as design_doc
        else:
            doc_type = "design_doc"  # Code files stored as design_doc
