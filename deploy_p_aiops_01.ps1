...
param(
    [string]$Server = "root@p-aiops-01",
    [string]$AppDir = "/aiops",
    [string]$Branch = "ui-1.0",
    [string]$RepoUrl = "https://github.com/maftabmirza/remediation-engine.git"
)

Write-Host "=========================================" -ForegroundColor Cyan
Write-Host "Deploying AIOps to p-aiops-01" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "Target Server: $Server" -ForegroundColor Yellow
Write-Host "Deployment Directory: $AppDir" -ForegroundColor Yellow
...
try {
    # Use ssh to execute the remote script
    ...
    Write-Host "1. Verify application is accessible" -ForegroundColor White
    Write-Host "2. Check logs: ssh $Server 'cd $AppDir && docker-compose logs -f remediation-engine'" -ForegroundColor White
    Write-Host "3. Verify database migrations: ssh $Server 'cd $AppDir && docker-compose exec remediation-engine atlas migrate status --url `"`$DATABASE_URL`" --dir `"file://atlas/migrations`"'" -ForegroundColor White
    Write-Host ""
    Write-Host "If Atlas migrations failed:" -ForegroundColor Yellow
    Write-Host "  - Check atlas/migrations directory exists" -ForegroundColor White
    Write-Host "  - Verify schema/schema.sql is present" -ForegroundColor White
    Write-Host "  - Review entrypoint.sh for Atlas migration logic" -ForegroundColor White
    ...
    Write-Host "1. Verify SSH connectivity: ssh $Server 'echo Connected successfully'" -ForegroundColor White
    Write-Host "2. Check if git is installed on server: ssh $Server 'git --version'" -ForegroundColor White
    Write-Host "3. Verify Docker is running: ssh $Server 'docker ps'" -ForegroundColor White
    Write-Host ""
    exit 1
}
