...
def create_index_safe_with_column_check(index_name, table_name, columns, **kw):
    ...
    for col in columns:
        if not column_exists(table_name, col):
            print(f"Skipping index {index_name}: column {col} does not exist in {table_name}")
            return
    # If all columns exist, create index safely
...
def upgrade() -> None:
    ...
    drop_table_safe('apscheduler_jobs')
    ...
    create_index_safe_with_column_check(op.f('ix_alerts_correlation_id'), 'alerts', ['correlation_id'], unique=False)
