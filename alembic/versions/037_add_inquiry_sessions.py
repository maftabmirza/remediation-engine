...
def upgrade():
    ...
    op.create_index('ix_inquiry_sessions_updated_at', 'inquiry_sessions', ['updated_at'])
    
    # Add session_id column to inquiry_results
    op.add_column('inquiry_results', 
        sa.Column('session_id', UUID(as_uuid=True), sa.ForeignKey('inquiry_sessions.id', ondelete='CASCADE'), nullable=True)
    )
    op.create_index('ix_inquiry_results_session_id', 'inquiry_results', ['session_id'])


def downgrade():
    """Drop inquiry_sessions table and session_id column."""
    op.drop_index('ix_inquiry_results_session_id')
    op.drop_column('inquiry_results', 'session_id')
    op.drop_index('ix_inquiry_sessions_updated_at')
    op.drop_index('ix_inquiry_sessions_user_id')
